...
| 约束                 | 说明                                                                 |
| -------------------- | -------------------------------------------------------------------- | ------------------------------- |
| 仅搜一个库           | 由 `spec_lock.md` 的 `icons.library` 决定（`simple-icons` 可额外搜） |
| 每次 1-3 个关键词    | `grep "keyword1\|keyword2\|keyword3"` 合并查询，不要分多次 `ls`      |
| 最多 3 轮            | 同一概念换变体拼写最多试 3 次，未命中即用 inventory 最近似项         |
| 严禁 `ls` 全量枚举   | `ls templates/icons/<lib>/` 不加 `                                   | grep` 将输出数千文件 → 绝对禁止 |
| 禁止在浏览模板时搜索 | 只有 Executor 在生成具体某页时才搜                                   |
