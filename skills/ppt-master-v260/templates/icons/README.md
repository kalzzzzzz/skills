...
Use `ls | grep` — zero token cost. **Hard limit: search only the one library chosen in `spec_lock.md`. Never enumerate all libraries. Never `ls` without `| grep` — a bare `ls` on any library outputs thousands of files and will trigger an infinite search loop.**
...
> ⚠️ If `grep` returns empty after 2-3 variant attempts, stop searching. Pick the closest match from `icons.inventory` in `spec_lock.md` instead.
